...
#ifndef SPIDS_STM32F7_SPI_DRIVER_H
#define SPIDS_STM32F7_SPI_DRIVER_H
...
#define SPIDS_STM32F7_SPI_MAX_FREQ		25000000			// Hz - Maximum data transfer frequency
#define SPIDS_STM32F7_SPI_INIT_FREQ		400000				// Hz - Maximum frequency used for initialization
#define SPIDS_STM32F7_SPI_CPOL 			SPI_POLARITY_HIGH	// Idle high
#define SPIDS_STM32F7_SPI_CPHA			SPI_PHASE_2EDGE		// 2nd edge

// This is an SD spec... it REALLY should not be changed
#define SPIDS_STM32F7_SPI_CMD_FRAME_LENGTH 6
#define SPIDS_STM32F7_SPI_DATA_TOKEN_25		0xFC
#define SPIDS_STM32F7_SPI_DATA_TOKEN_READ	0xFE
#define SPIDS_STM32F7_SPI_DATA_TOKEN_STOP	0xFD

// Some rando defines
#define SPIDS_STM32F7_SPI_Default_Timeout	1000U
#define SPIDS_STM32F7_SPI_NUM_DUMMY_BYTES 	12000
#define SPIDS_STM32F7_SPI_DEF_BLOCKLEN 		512		// This shouldnt change either
...
typedef enum{
	SPIDS_STM32F7_SPI_CODE_NOM = 0,
	SPIDS_STM32F7_SPI_CODE_TIMEOUT
}SPIDS_STM32F7_SPI_CodeTypeDef;
...
}SPIDS_STM32F7_SPI_Settings_TypeDef;			// This settings structure can be used to pass some general SPI settings into the driver (more or less)

// Since I can't have a global-scope default SPI settings object (Atollic can't find the program file if I try the following:
// const SPIDS_STM32F7_SPI_Settings_TypeDef hello2 = {0, 0, 0, 0}; ) then I will use a default initializer function
...
DSTATUS SPIDS_STM32F7_SPI_disk_initialize(BYTE pdrv, SPIDS_STM32F7_SPI_Settings_TypeDef * settings);                     							/*!< Initialize Disk Drive                     */
DSTATUS SPIDS_STM32F7_SPI_disk_status(BYTE pdrv, SPIDS_STM32F7_SPI_Settings_TypeDef * settings);                     								/*!< Get Disk Status                           */
DRESULT SPIDS_STM32F7_SPI_disk_read(BYTE pdrv, BYTE* buff, DWORD sector, UINT count, SPIDS_STM32F7_SPI_Settings_TypeDef * settings);   /*!< Read Sector(s)                            */
//#if _USE_WRITE == 1
	DRESULT SPIDS_STM32F7_SPI_disk_write(BYTE pdrv, const BYTE* buff, DWORD sector, UINT count, SPIDS_STM32F7_SPI_Settings_TypeDef * settings); 		/*!< Write Sector(s) when _USE_WRITE = 0       */
//#endif /* _USE_WRITE == 1 */
//#if _USE_IOCTL == 1
	DRESULT SPIDS_STM32F7_SPI_disk_ioctl(BYTE pdrv, BYTE cmd, void* buff, SPIDS_STM32F7_SPI_Settings_TypeDef * settings);              					/*!< I/O control operation when _USE_IOCTL = 1 */
//#endif /* _USE_IOCTL == 1 */
