...
This is an SD interface that uses the SPI peripherals on an STM32F4 platform. 
...
uint8_t ALL_HIGH_BLOCKLEN[SPIDS_STM32F4_SPI_DEF_BLOCKLEN];
...
//SPIDS_DiskioDriver_Typedef SPIDS_STM32F4_SPI_Driver;


//const SPIDS_STM32F4_SPI_Settings_TypeDef hello2 = {0, 0, 0, 0};
...
DSTATUS SPIDS_STM32F4_SPI_disk_initialize(BYTE pdrv, SPIDS_STM32F4_SPI_Settings_TypeDef * settings)
{
...
//	SPIDS_STM32F4_SPI_CMD_SET_BLOCKLEN(settings, 0x200);
...
DSTATUS SPIDS_STM32F4_SPI_disk_status(BYTE pdrv, SPIDS_STM32F4_SPI_Settings_TypeDef * settings)
{
...
//	SPIDS_STM32F4_SPI_CMD_SEND_STATUS(settings);
	SPIDS_STM32F4_SPI_CMD(settings, SDMMC_CMD_SEND_STATUS, 0, 0);
	SPIDS_STM32F4_SPI_GET_R2(settings, &r2_resp, 10);
	if(result != SPIDS_STM32F4_SPI_CODE_NOM)	// Check to see if there was a timeout or something...
	{
		HAL_GPIO_WritePin(settings->CS_GPIO_Port, settings->CS_Pin, 1);
		return STA_NOINIT;
	}
...
DRESULT SPIDS_STM32F4_SPI_disk_read(BYTE pdrv, BYTE* buff, DWORD sector, UINT count, SPIDS_STM32F4_SPI_Settings_TypeDef * settings)
{
	...
	uint16_t r2_resp = 0xFFFF;
	SPIDS_STM32F4_SPI_CodeTypeDef	result = SPIDS_STM32F4_SPI_CODE_NOM;
	const uint16_t blocklen = SPIDS_STM32F4_SPI_DEF_BLOCKLEN;
...
//#if _USE_WRITE == 1
DRESULT SPIDS_STM32F4_SPI_disk_write(BYTE pdrv, const BYTE* buff, DWORD sector, UINT count, SPIDS_STM32F4_SPI_Settings_TypeDef * settings)
{
	uint32_t start_address = 0x00;
	uint8_t r1_resp = 0xFF;
	uint16_t r2_resp = 0xFFFF;
...
//#if _USE_IOCTL == 1
DRESULT SPIDS_STM32F4_SPI_disk_ioctl(BYTE pdrv, BYTE cmd, void* buff, SPIDS_STM32F4_SPI_Settings_TypeDef * settings)
{
	WORD sector_size = 0;
	DWORD num_sectors = 0;
...
uint32_t SPIDS_STM32F4_SPI_Get_Num_Sectors(SPIDS_STM32F4_SPI_Settings_TypeDef * settings)
{
	...
	HAL_SPI_TransmitReceive(settings->hspi, &ALL_HIGH, &r1_resp, 1, settings->Timeout);
	if(r1_resp != SPIDS_STM32F4_SPI_DATA_TOKEN_25)
	{
...
////		num_sectors += 1; // Maybe... according to the SD spec page 87... but seems fishy. Can't hurt to tell fatfs one less....
		...
		// Having further explored SD cards in the USB quest I realized that the formular on pg 87 goes like this:
		// memory_capacity = (C_SIZE + 1)*512 K bytes.
		// Where C_size is the OR'd together value from above. We might assume that K measn 1024, but it is safer to go with 1000 so we will do that for now...
		// In the HC hardware the block size is always 512 bytes, so then we realize that:
